...
DOTTED_EMILY_EMAIL = "daniel.reed25@gmail.com"
...
pcp_zip_and_fax_update_then_transfer_for_id_card = Scenario(
    name="pcp_zip_and_fax_update_then_transfer_for_id_card",
    flow="pcp",
    user_turns=[
        "I want to get the list of providers in my area.",
        "My name is Jim citizen.",
        "Yes.",
        "My member... just hold on. My member ID is m three one zero one eight eight.",
        "It's thirtieth July nineteen seventy seven.",
        "My name is James Wilson.",
        "and my date of birth is thirtieth July three zero... thirtieth July nineteen seventy seven",
        "James Wilson.",
        "Yes.",
        "I am the I am the sub... I... I'm calling for myself.",
        "I am I am looking for I am looking for a pediatrician.",
        "No. I want to update it.",
        "One six seven eight three.",
        "Can you send me, , by fax, please?",
        "Yes. That is correct.",
        "Yes. But tell me one thing. Can I change my fax number now?",
        "Two three one, triple five, three two one one.",
        "Yes.",
        "No. But I lost my credit ID card. Can you help me with the new one?",
        "That sounds interesting, but I lost my ID card. Can you help me to get a new one?",
        "I have lost my ID card. Can you please help me get get a new one?",
        "Can you just transfer me can you just transfer me to the right department?",
    ],
    turn_expectations={
        # 0: TurnExpectation(ai_contains=[r"first\s*name"]),
        # 2: TurnExpectation(ai_contains=[r"member\s*id"]),
        # 4: TurnExpectation(ai_contains=[r"date\s*of\s*birth|birth\s*date|dob"]),
        # 10: TurnExpectation(ai_contains=[r"zip\s*code|correct\?"], slot_awaiting="zip_code"),
        # 14: TurnExpectation(ai_contains=[r"fax|email"]),
        # 18: TurnExpectation(ai_contains=[r"fax\s*number|updated fax"], slot_awaiting="fax"),
    },
    expect=Expected(
        completed=True,
        escalated=True,
        ...
        final_state={
            ...
            "delivery_method": "fax",
            "zip_code_updated": True,
            "fax": digits_equal("2155553211"),
        },
        last_ai_contains=[
            r"live representative",
            r"reference number",
            r"Please hold",
        ],
    ),
    notes=(
        "Provider lookup with mixed verification turbulence: member first gives a name, "
        "then the member ID and DOB are corrected after mismatch. The ZIP is updated, "
        "the fax number is updated, the provider list is sent, then the member asks for "
        "a replacement ID card and finally requests transfer to the right department. "
        "The flow should still close via live transfer with a reference number."
    ),
)
...
pcp_email_and_fax_delivery = Scenario(
    name="pcp_email_and_fax_delivery",
    flow="pcp",
    user_turns=PCP_VERIFY
    + [
        "yes that's correct",  # ZIP on file
        "please send it by both email and fax",
        "fax please",  # choose one channel after clarification
        "yes that's correct",  # fax on file
        "no thanks",  # decline benefits offer
        "no thank you",  # decline Care Coach
        "no, that's everything, thanks",  # close
    ],
    turn_expectations={
        9: TurnExpectation(ai_contains=[r"fax.*email|email.*fax"], slot_awaiting="delivery_method"),
        10: TurnExpectation(ai_contains=[r"fax.*(right|correct)|right.*fax"], slot_awaiting="fax_confirmed"),
    },
    expect=Expected(
        completed=True,
        escalated=False,
    ...
    notes=(
        "A combined provider-list request must re-ask the member to choose exactly one "
        "channel before reading back a delivery contact."
    ),
)

pcp_dotted_email_correction = Scenario(
    name="pcp_dotted_email_correction",
    flow="pcp",
    mutating=True,
    user_turns=PCP_VERIFY
    + [
        ...
        "the correct id is daniel dot reed two five at gmail dot com",
        "no it is daniel dot reed 25 at gmail dot com",
        DOTTED_EMILY_EMAIL,
        "yes that's correct",  # confirm dotted address
        "no thanks",  # decline benefits offer
        "no thank you",  # decline Care Coach
        "no, that's everything, thanks",  # close
    ],
    turn_expectations={
        11: TurnExpectation(
            ai_contains=[r"daniel\s+dot\s+reed\s+two\s+five\s+at\s+gmail\s+dot\s+com"],
            slot_awaiting="email_confirmed",
        ),
        12: TurnExpectation(
            ai_contains=[r"(correct|updated).*email|email.*(correct|updated)"], slot_awaiting="email"
        ),
    },
    expect=Expected(
        completed=True,
        ...
        final_state={
            "provider_list_sent": True,
            "email": contains(DOTTED_EMILY_EMAIL),
        },
    ),
    post_checks=[sf_field_check("M907503", "email", DOTTED_EMILY_EMAIL)],
    notes=(
        "A spoken local-part dot must survive extraction and read-back. A later 'no' "
        "with the same spoken address must request a corrected email instead of dispatching."
    ),
)
...
claim_status_refusal_end_conversation = Scenario(
    name="claim_status_refusal_end_conversation",
    flow="claim",
    user_turns=[
        "I want to check the claim status. Can you help me with that today?",
        "No. You cannot get my first name.",
        "Please check my claim status today.",
        "How are you doing today?",
        "Can we end this conversation?",
    ],
    turn_expectations={
        # 0: TurnExpectation(ai_contains=[r"first\s*name"]),
        # 2: TurnExpectation(ai_contains=[r"first\s*name"]),
        # 3: TurnExpectation(ai_contains=[r"first\s*name|doing well"]),
    },
    expect=Expected(
        completed=True,
        escalated=True,
    ...
    notes=(
        "Regression: caller refuses to provide first name for a claim-status check, "
        "repeats the request, asks a small-talk question, then asks to end the call. "
        "The agent should continue to refuse identity verification, transfer to a live "
        "representative, and provide a reference number while ending politely."
    ),
)

claim_status_first_name_repeat_loop = Scenario(
    name="claim_status_first_name_repeat_loop",
    flow="claim",
    timeout_s=360,
    user_turns=[
        "I want to check the pictures. Can you help me with that today?",
        "Yes. It's correct. I want to check the claim status.",
        "Can you also check me the first name of my name?",
        "Can you repeat?",
        "Can you repeat?",
        "No, that's all. Thank you.",
    ],
    turn_expectations={
        # 1: TurnExpectation(ai_contains=[r"first\s*name"]),
        # 2: TurnExpectation(ai_contains=[r"first\s*name"]),
        # 3: TurnExpectation(ai_contains=[r"first\s*name"]),
    },
    expect=Expected(
        completed=True,
        escalated=False,
    ...
    notes=(
        "Claim-status intake should politely restate the first-name requirement and "
        "continue to ask for it when the member asks for the 'first name of my name' "
        "and then asks to repeat twice. The conversation should end gracefully once "
        "the member declines further verification, without escalating or misrouting."
    ),
)
...
notification_both_channels_requires_choice = Scenario(
    name="notification_both_channels_requires_choice",
    flow="claim",
    timeout_s=360,
    user_turns=_CLAIM_TO_NOTIFICATION
    + [
        "send updates by both SMS and email",
        "text messages please",
        "yes that's correct",  # phone on file
        "how long will it take to finalize the request?",
        "email them to me",  # N2 channel
        "no, that's it. Thanks!",  # close
    ],
    turn_expectations={
        12: TurnExpectation(
            ai_contains=[r"(sms|text).*email|email.*(sms|text)"], slot_awaiting="notification_method"
        ),
        13: TurnExpectation(
            ai_contains=[r"(sms|text).*email|email.*(sms|text)"], slot_awaiting="notification_method"
        ),
    ...
    notes=(
        "Notification preferences support one channel. A request for both SMS and email "
        "must re-ask for a choice rather than selecting a channel on the member's behalf."
    ),
)
...
SCENARIOS: list[Scenario] = [
    ...
    pcp_zip_and_fax_update_then_transfer_for_id_card,  # 6a — ZIP/fax update, then transfer for ID card
    pcp_email_update,  # 7  (mutating)
    pcp_email_and_fax_delivery,  # 7a — sequential dual-channel delivery
    pcp_dotted_email_correction,  # 7b — dotted local-part read-back and declined repeat
    # B. Verification escalations
    ...
    # E. Claim flow
    claim_happy_path,  # 21
    notification_both_channels_requires_choice,  # 21a — do not choose SMS/email for caller
    claim_upload_only,  # 22
    claim_guide_only,  # 23
    claim_no_proceed,  # 24
    claim_status_refusal_end_conversation,  # 24a — caller refuses first-name verification, then ends call
    claim_status_first_name_repeat_loop,  # 24b — claim-status request
