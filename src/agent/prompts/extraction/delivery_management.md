...
FIELDS — every name below is a key of `extracted{}`
  delivery_method  "fax" | "email" | "both"
    ...
    fax", "fax and email", "send it to both"), extract delivery_method="both".
    The delivery agent supports one delivery channel per request and will ask
    the caller to choose fax or email.
  ...
  email  valid email string (must contain "@" and a domain)
    ...
    remove it or merge the words on either side.
